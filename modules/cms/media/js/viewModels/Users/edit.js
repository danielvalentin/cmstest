define([
	...
	return function(data){
		...
		self.setAvatarId = function(files){
			var file = files[0];
			self.user.avatar.id(file.id.toString());
			self.user.avatar.img.medium(file.medium());
			self.user.avatar.img.mini(file.mini()); 
		};
		
		self.user.avatar.id.subscribe(function(newvat){
			console.log(newvat);
		});
		
		self.clearAvatar = function(){
			self.user.avatar.id(0);
		};
